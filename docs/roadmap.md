...
- ✅ **Advancements** — a guided "Great Work" progression tree: a 24-node advancement
  tree (`data/echoes/advancement/great_work/`, generated by `gen_advancements.py`) that
  traces the whole spine — refine → generate → carry/bank → spend → radiation → wireless
  → the octave climb → transmutation — with goal/challenge capstones and XP rewards. See
  [`docs/wiki/The-Great-Work.md`](wiki/The-Great-Work.md).
