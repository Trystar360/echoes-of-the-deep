...
public class ConfigScreen extends HandledScreen<ConfigScreenHandler> {
    ...
    private ButtonWidget channelBtn, octaveBtn, redstoneBtn, tuningABtn, tuningBBtn;
    private final ButtonWidget[] sideBtns = new ButtonWidget[6];

    public ConfigScreen(ConfigScreenHandler handler, PlayerInventory inv, Text title) {
        super(handler, inv, title);
        this.spec = handler.spec();
        this.backgroundWidth = 214;
        this.backgroundHeight = 30 + rowCount() * 24 + 8;
        this.playerInventoryTitleY = -1000; // hide "Inventory" label (no slots)
        this.titleX = 8;
    }
    ...
    protected void init() {
        ...
        if (spec.redstone()) {
            redstoneBtn = addButton(x() + 96, y, 108, Text.empty(), ConfigScreenHandler.B_REDSTONE);
            y += 24;
        }
        if (spec.sides()) {
            for (int i = 0; i < 6; i++) {
                sideBtns[i] = addButton(x() + 70 + i * 23, y, 21, Text.literal(FACE[i]),
    ...
    private ButtonWidget addValueRow(int y, int downId, int upId) {
        addButton(x() + 96, y, 20, Text.literal("-"), downId);
        ButtonWidget value = ButtonWidget.builder(Text.empty(), b -> {})
                .dimensions(x() + 118, y, 64, 20).build();
        value.active = false;
        addDrawableChild(value);
        addButton(x() + 184, y, 20, Text.literal("+"), upId);
        return value;
    }

    private ButtonWidget addButton(int bx, int by, int w, Text label, int buttonId) {
    ...
    private void refreshLabels() {
        if (channelBtn != null) {
            ...
            channelBtn.setMessage(Text.translatable("color.minecraft." + dye.getName()));
        }
        if (octaveBtn != null) {
            int oc = Math.floorMod(handler.prop(ConfigScreenHandler.P_OCTAVE), ROMAN.length);
            octaveBtn.setMessage(Text.literal(ROMAN[oc]));
        }
        if (redstoneBtn != null) {
            RedstoneMode m = RedstoneMode.byId(handler.prop(ConfigScreenHandler.P_REDSTONE));
            redstoneBtn.setMessage(Text.translatable(m.translationKey()));
        }
        for (int i = 0; i < 6; i++) {
            if (sideBtns[i] == null) continue;
            SideMode sm = SideMode.byId(handler.prop(ConfigScreenHandler.P_SIDE0 + i));
            sideBtns[i].setMessage(Text.literal(FACE[i]).formatted(sideColor(sm)));
        }
        if (tuningABtn != null) tuningABtn.setMessage(Text.literal(String.valueOf(handler.prop(ConfigScreenHandler.P_TUNING_A))));
        if (tuningBBtn != null) tuningBBtn.setMessage(Text.literal(String.valueOf(handler.prop(ConfigScreenHandler.P_TUNING_B))));
    }

    private static Formatting sideColor(SideMode m) {
        return switch (m) {
            case DISABLED -> Formatting.DARK_GRAY;
            case INPUT -> Formatting.AQUA;
            case OUTPUT -> Formatting.GOLD;
            case BOTH -> Formatting.GREEN;
        };
    }

    /** Left-hand label for each row, drawn relative to its button's Y. */
    private void drawRowTitles(DrawContext ctx) {
        int color = 0xC8E6E6;
    ...
    private void label(DrawContext ctx, String key, int rowY, int color) {
        ctx.drawText(textRenderer, Text.translatable(key), x() + 10, rowY + 6, color, false);
    }

    private void labelKey(DrawContext ctx, String key, int rowY, int color) {
        ctx.drawText(textRenderer, Text.translatable(key), x() + 10, rowY + 6, color, false);
    }
}
