#!/bin/bash
# Verifier v13 — Echoes of the Deep "fully loaded" acceptance checks
# Usage: verifier/v13/verify.sh ; exit 0 = PASS
cd "$(dirname "$0")/../.." || exit 1
fail=0
...
python3 verifier/v13/check_assets.py
...
python3 verifier/v13/check_textures32.py
...
[ -f src/main/java/com/echoes/block/ResonantCondenserBlock.java ] && [ -f src/main/java/com/echoes/block/entity/ResonantCondenserBlockEntity.java ] && t34=0 || t34=1; check "ProjectE-style condenser (Resonant Condenser)" $t34
grep -q "RESONANT_CONDENSER" src/main/java/com/echoes/registry/ModBlocks.java && grep -q "RESONANT_CONDENSER" src/main/java/com/echoes/registry/ModBlockEntities.java && t35=0 || t35=1; check "condenser block+entity registered" $t35
[ -f src/main/resources/data/echoes/recipe/resonant_condenser.json ] && [ -f src/main/resources/data/echoes/loot_table/blocks/resonant_condenser.json ] && [ -f src/main/resources/assets/echoes/blockstates/resonant_condenser.json ]; check "condenser recipe+loot+blockstate" $?
grep -q "TransmutationState" src/main/java/com/echoes/block/entity/ResonantCondenserBlockEntity.java && grep -q "LightValues" src/main/java/com/echoes/block/entity/ResonantCondenserBlockEntity.java && t36=0 || t36=1; check "condenser bound to transmutation account" $t36
grep -q "block.echoes.resonant_condenser" src/main/resources/assets/echoes/lang/en_us.json && grep -q "message.echoes.condenser.target" src/main/resources/assets/echoes/lang/en_us.json && t37=0 || t37=1; check "condenser lang keys" $t37
grep -q "Resonant Condenser" docs/mechanics-guide.md && t38=0 || t38=1; check "docs cover the condenser" $t38
for j in build/libs/echoes-of-the-deep-[0-9]*.jar; do unzip -l "$j" 2>/dev/null | grep -q "com/echoes/block/entity/ResonantCondenserBlockEntity.class" && t39=0 && break; t39=1; done; check "condenser in shipped jar" $t39

# --- v12: AE2-style Wave Terminal (remote channel storage) ---
[ -f src/main/java/com/echoes/item/WaveTerminalItem.java ] && t40=0 || t40=1; check "AE2-style wireless terminal (Wave Terminal)" $t40
grep -q '\"wave_terminal\"' src/main/java/com/echoes/registry/ModItems.java && grep -q "WAVE_TERMINAL" src/main/java/com/echoes/registry/ModItemGroups.java && t41=0 || t41=1; check "terminal item registered + in creative tab" $t41
grep -q "devicesOnChannel" src/main/java/com/echoes/wireless/WirelessNetworkManager.java && grep -q "devicesOnChannel" src/main/java/com/echoes/item/WaveTerminalItem.java && t42=0 || t42=1; check "terminal reads live channel roster" $t42
grep -q "canAccess" src/main/java/com/echoes/item/WaveTerminalItem.java && grep -q "redstoneMode().allows" src/main/java/com/echoes/item/WaveTerminalItem.java && t43=0 || t43=1; check "terminal honors locks + redstone gating" $t43
[ -f src/main/resources/data/echoes/recipe/wave_terminal.json ] && [ -f src/main/resources/assets/echoes/items/wave_terminal.json ] && [ -f src/main/resources/assets/echoes/models/item/wave_terminal.json ] && [ -f src/main/resources/assets/echoes/textures/item/wave_terminal.png ]; check "terminal recipe+item def+model+texture" $?
grep -q "message.echoes.terminal.bound" src/main/resources/assets/echoes/lang/en_us.json && grep -q "tooltip.echoes.desc.wave_terminal" src/main/resources/assets/echoes/lang/en_us.json && t44=0 || t44=1; check "terminal lang keys" $t44
grep -q "Wave Terminal" docs/mechanics-guide.md && t45=0 || t45=1; check "docs cover the wave terminal" $t45
ls build/test-results/test/TEST-com.echoes.data.ResourcesDataTest.xml >/dev/null 2>&1 && ! grep -q 'failures="[1-9]\|errors="[1-9]' build/test-results/test/TEST-com.echoes.data.ResourcesDataTest.xml; check "resource wiring tests green (13 tests)" $?
for j in build/libs/echoes-of-the-deep-[0-9]*.jar; do unzip -l "$j" 2>/dev/null | grep -q "com/echoes/item/WaveTerminalItem.class" && t46=0 && break; t46=1; done; check "terminal in shipped jar" $t46

# --- v13: TD-style per-relay servo filters ---
[ -f src/main/java/com/echoes/wireless/ServoFilter.java ] && t47=0 || t47=1; check "servo filter core (ServoFilter)" $t47
grep -q "extractionFilter" src/main/java/com/echoes/wireless/WirelessDevice.java && grep -q "extractionFilter" src/main/java/com/echoes/block/entity/ResonantRelayBlockEntity.java && t48=0 || t48=1; check "relays expose a servo extraction filter" $t48
grep -q "extractionFilter" src/main/java/com/echoes/wireless/WirelessNetworkManager.java && t49=0 || t49=1; check "manager applies per-sender servo filters" $t49
grep -q "toggleFilter" src/main/java/com/echoes/block/ResonantRelayBlock.java && t50=0 || t50=1; check "item-click servo filter interaction" $t50
grep -q "message.echoes.servo.added" src/main/resources/assets/echoes/lang/en_us.json && grep -q "message.echoes.servo.full" src/main/resources/assets/echoes/lang/en_us.json && t51=0 || t51=1; check "servo lang keys" $t51
grep -q "servo filter" docs/mechanics-guide.md && t52=0 || t52=1; check "docs cover servo filters" $t52
ls build/test-results/test/TEST-com.echoes.data.ServoFilterTest.xml >/dev/null 2>&1 && ! grep -q 'failures="[1-9]\|errors="[1-9]' build/test-results/test/TEST-com.echoes.data.ServoFilterTest.xml; check "servo filter tests green" $?
for j in build/libs/echoes-of-the-deep-[0-9]*.jar; do unzip -l "$j" 2>/dev/null | grep -q "com/echoes/wireless/ServoFilter.class" && t53=0 && break; t53=1; done; check "servo filter in shipped jar" $t53
